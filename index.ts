...
class ExampleAugmentOSApp extends TpaServer {
  ...
  private intervalDelta: number;
  private currentSongDelay: number;
  ...
  private showInstructions(session: TpaSession): void {
    session.layouts.showTextWall(
      ...
      "Catalog: 'next', 'previous', 'select'.\n" +
      "Left pedal: auto/manual. Right: fwd, middle: back.\n", { durationMs: 3000 }
    );
  }
  ...
  private selectSong(session: TpaSession): void {
    if (this.currentState === AppState.CATALOG_VIEW) {
      ...
      this.image_index = 0;
      // clear text
      session.layouts.showDoubleTextWall("", "", {durationMs: 0});
    ...
    process.stdin.on('data', (key: Buffer) => {
      ...
      if (keyPress.startsWith(';')) {
        if (this.appMode === AppMode.AUTO) {
          ...
          // set the current standard interval
          this.currentSongDelay = this.getCurrentSong().intervalMs;
      ...
      if (keyPress.startsWith(']') || keyPress.startsWith('[')) {
        ...
        if (this.appMode === AppMode.MANUAL) {
          ...
          if (isForward) {
            console.log('Speeding up (auto mode)');
